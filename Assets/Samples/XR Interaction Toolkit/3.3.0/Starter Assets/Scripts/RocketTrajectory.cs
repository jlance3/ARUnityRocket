...
public class RocketTrajectory : MonoBehaviour
{
    ...
    Transform FindClosestTargetWithTag()
    {
        //Finds all objects with tag
        GameObject[] rocketPads = GameObject.FindGameObjectsWithTag("RocketPad");
        ...
        foreach (GameObject potentialTarget in rocketPads)
        {
            Vector3 directionToTarget = potentialTarget.transform.position - currentPosition;
            // Use sqrMagnitude for faster distance calculation (avoids a square root operation)
            float dSqrToTarget = directionToTarget.sqrMagnitude;

            if (dSqrToTarget < closestDistanceSqr)
            {
                closestDistanceSqr = dSqrToTarget;
                closestPad = potentialTarget.transform;
            }
        }
